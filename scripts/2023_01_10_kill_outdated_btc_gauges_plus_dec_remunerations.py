...
def main():
    """
    Kill outdated BTC gauges and send a batch of ~1.69m SDL (5% of total) for December 2022 
    ...
    # kill outdated BTC gauges
    gauges_to_kill = (
        "LiquidityGaugeV5_SaddleBTCPoolV2LPToken",
        "LiquidityGaugeV5_SaddleTBTCMetaPoolV3LPToken"
    )
    for gauge_name in gauges_to_kill:
        gauge = get_contract_from_deployment(1, gauge_name)
        gauge.set_killed(True, {"from":multisig.address})
