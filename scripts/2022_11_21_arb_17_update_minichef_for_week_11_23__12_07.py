...
def main():
    """
    Updates minichef parameters on Arbitrum network for week 11/23 - 12/07
    ...
    if minichef.poolInfo(3)[2] != 3717:
        minichef.set(3, 3717, ZERO_ADDRESS, False)

    # 4.92% saddleFraxUSDT
    assert minichef.lpToken(4) == "0x166680852ae9Dec3d63374c5eBf89E974448BFE9"
    if minichef.poolInfo(4)[2] != 492:
        minichef.set(4, 492, ZERO_ADDRESS, False)

    # 4.94% saddleFraxUSDs
    assert minichef.lpToken(5) == "0x1e491122f3C096392b40a4EA27aa1a29360d38a1"
    if minichef.poolInfo(5)[2] != 494:
        minichef.set(5, 494, ZERO_ADDRESS, False)
    ...
    assert minichef.totalAllocPoint() == 4703

    new_rate = math.floor(SIDECHAIN_TOTAL_EMISSION_RATE * 4703 / 10000)
