...
def main():
    """
    Updates minichef parameters on Arbitrum network for week 11/11 - 11/18
    ...
    # 0.01% for saddleArbUSD
    assert minichef.lpToken(1) == "0xc969dD0A7AB0F8a0C5A69C0839dB39b6C928bC08"
    if minichef.poolInfo(1)[2] != 1:
        minichef.set(1, 1, ZERO_ADDRESS, False)
    ...
    if minichef.poolInfo(3)[2] != 3424:
        minichef.set(3, 3424, ZERO_ADDRESS, False)

    # 4.56% saddleFraxUSDT
    assert minichef.lpToken(4) == "0x166680852ae9Dec3d63374c5eBf89E974448BFE9"
    if minichef.poolInfo(4)[2] != 456:
        minichef.set(4, 456, ZERO_ADDRESS, False)

    # 12.08% saddleFraxUSDs
    assert minichef.lpToken(5) == "0x1e491122f3C096392b40a4EA27aa1a29360d38a1"
    if minichef.poolInfo(5)[2] != 1208:
        minichef.set(5, 1208, ZERO_ADDRESS, False)
    ...
    # 0.03% saddleArbUSX-FRAXBP
    assert minichef.lpToken(8) == "0x721DaC7d5ACc8Aa62946fd583C1F999e1570b97D"
    if minichef.poolInfo(8)[2] != 3:
        minichef.set(8, 3, ZERO_ADDRESS, False)

    # Total allocation is 50.91% for Arbitrum
    assert minichef.totalAllocPoint() == 5092

    new_rate = math.floor(SIDECHAIN_TOTAL_EMISSION_RATE * 5092 / 10000)
