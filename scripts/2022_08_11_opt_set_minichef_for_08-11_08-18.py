...
def main():
    ...
    # 35.88% for Optimism: FraxBP
    assert minichef.lpToken(1) == "0xf74ebe6e5586275dc4CeD78F5DBEF31B1EfbE7a5"
    minichef.set(1, 3588, ZERO_ADDRESS, False, {"from": deployer})
    assert minichef.poolInfo(1)[2] == 3588

    # 5.77% for Optimism: Frax-optUSD
    ...
    assert minichef.poolInfo(6)[2] == 577

    # 5.03% for Optimism: Frax-USDT
    assert minichef.lpToken(3) == "0xb63d7B0D835ca6eFf89ab774498ed6dD0D71e93e"
    minichef.set(3, 503, ZERO_ADDRESS, False, {"from": deployer})
    assert minichef.poolInfo(3)[2] == 503

    # 5.03% for Optimism: Frax-SUSD
    assert minichef.lpToken(4) == "0x205c9B8c1fCa803B779b1eB4B887Aa0E00FE629F"
    minichef.set(4, 503, ZERO_ADDRESS, False, {"from": deployer})
    assert minichef.poolInfo(4)[2] == 503

    # 1.05% for Optimism: optUSD
    assert minichef.lpToken(5) == "0xcCf860874cbF2d615192a4C4455580B4d622D3B9"
    minichef.set(5, 105, ZERO_ADDRESS, False, {"from": deployer})
    assert minichef.poolInfo(5)[2] == 105
    ...
    # Total allocation is 52.76% for Arbitrum

    assert minichef.totalAllocPoint() == 5276

    new_rate = math.floor(SIDECHAIN_TOTAL_EMISSION_RATE * 5276 / 10000)
    print(new_rate)
