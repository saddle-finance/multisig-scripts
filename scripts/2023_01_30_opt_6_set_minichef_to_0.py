import math

from ape_safe import ApeSafe
from brownie import ZERO_ADDRESS, Contract, accounts, history, network

from helpers import (CHAIN_IDS, GNOSIS_SAFE_BASE_URLS, MINICHEF_ADDRESSES,
                     OPS_MULTISIG_ADDRESSES)
from scripts.utils import confirm_posting_transaction

TARGET_NETWORK = "OPTIMISM"


def main():
    """
    Sets minchef reward weight to 0
    Vote: https://snapshot.org/#/saddlefinance.eth/proposal/0xfbc83d80649b7f7cd4e0c430e90b412cfdea9c85e8584245247ef05b14eeaf7b
    """
    print(f"You are using the '{network.show_active()}' network")
    assert (
        network.chain.id == CHAIN_IDS[TARGET_NETWORK]
    ), f"Not on {TARGET_NETWORK} network"

    deployer = accounts.load("deployer")

    ##### Update Minichef weights #####
    multisig = ApeSafe(
        OPS_MULTISIG_ADDRESSES[CHAIN_IDS[TARGET_NETWORK]],
        GNOSIS_SAFE_BASE_URLS[CHAIN_IDS[TARGET_NETWORK]],
        multisend="0x998739BFdAAdde7C933B942a68053933098f9EDa",
    )
    minichef = Contract.from_explorer(MINICHEF_ADDRESSES[CHAIN_IDS[TARGET_NETWORK]])

    # Mass update pools to checkpoint existing stakers
    pids = [1, 2, 3, 4, 5, 7]
    minichef.massUpdatePools(pids, {'from': multisig.account})

    # 0.01 for placeholder
    if minichef.poolInfo(0)[2] != 1:
        minichef.set(0, 1, ZERO_ADDRESS, False, {'from': multisig.account})

    # 0.00% for Optimism: FraxBP
    assert minichef.lpToken(1) == "0xf74ebe6e5586275dc4CeD78F5DBEF31B1EfbE7a5"
    if minichef.poolInfo(1)[2] != 0:
        minichef.set(1, 0, ZERO_ADDRESS, False, {'from': multisig.account})

    # 0.00% for Optimism: FraxBP-optUSD
    assert minichef.lpToken(2) == "0xfF5fa61Eb9b5cDD63bdFa16EF029d5313457925A"
    if minichef.poolInfo(2)[2] != 0:
        minichef.set(2, 0, ZERO_ADDRESS, False, {'from': multisig.account})

    # 0.00% for Optimism: FraxBP-USDT
    assert minichef.lpToken(3) == "0xb63d7B0D835ca6eFf89ab774498ed6dD0D71e93e"
    if minichef.poolInfo(3)[2] != 0:
        minichef.set(3, 0, ZERO_ADDRESS, False, {'from': multisig.account})

    # 0.00% for Optimism: FraxBP-SUSD
    assert minichef.lpToken(4) == "0x205c9B8c1fCa803B779b1eB4B887Aa0E00FE629F"
    if minichef.poolInfo(4)[2] != 0:
        minichef.set(4, 0, ZERO_ADDRESS, False, {'from': multisig.account})

    # 0.00% for Optimism: optUSD
    assert minichef.lpToken(5) == "0xcCf860874cbF2d615192a4C4455580B4d622D3B9"
    if minichef.poolInfo(5)[2] != 0:
        minichef.set(5, 0, ZERO_ADDRESS, False, {'from': multisig.account})

    # Should always be 0.00% for pid 6.
    # This is not a LP token address. Pool address was added by mistake.
    assert minichef.lpToken(6) == "0xc55E8C79e5A6c3216D4023769559D06fa9A7732e"
    assert minichef.poolInfo(6)[2] == 0

    # 0.00% for Optimism: saddleOptUSX-FRAXBP
    assert minichef.lpToken(7) == "0xf349fB2b5eD45864e1d9ad34a483Eb37aC6e0034"
    if minichef.poolInfo(7)[2] != 0:
        minichef.set(7, 0, ZERO_ADDRESS, False, {'from': multisig.account})

    # Set new reward weight to 0
    new_rate = math.floor(0)
    minichef.setSaddlePerSecond(new_rate, {'from': multisig.account})

    # Check state variables are updated correctly
    assert minichef.poolInfo(0)[2] == 1
    assert minichef.poolInfo(1)[2] == 0
    assert minichef.poolInfo(2)[2] == 0
    assert minichef.poolInfo(3)[2] == 0
    assert minichef.poolInfo(4)[2] == 0
    assert minichef.poolInfo(5)[2] == 0
    assert minichef.poolInfo(6)[2] == 0
    assert minichef.poolInfo(7)[2] == 0
    assert minichef.totalAllocPoint() == 1
    assert minichef.saddlePerSecond() == new_rate

    print(f"New MiniChef rate on {TARGET_NETWORK} is {new_rate}")

    # combine history into multisend txn
    safe_tx = multisig.multisend_from_receipts()
    safe_tx.safe_nonce = 6

    # sign with private key
    safe_tx.sign(deployer.private_key)
    multisig.preview(safe_tx)

    confirm_posting_transaction(multisig, safe_tx)
